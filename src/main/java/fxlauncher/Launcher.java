...
public class Launcher extends Application {
      ...
      runAndWait(() -> {
        try {
          if (Application.class.isAssignableFrom(appClass)) {
            ...
            throw new IllegalArgumentException(String.format(Constants.getString("Error.Application.Create.1"),
                                                             appClass));
          }
        } catch (Throwable t) {
   ...
   * update.
   * <p>
   * Note that updates will only be detected if the application was actually launched with
   * FXLauncher.
   *
    ...
    new Thread(() -> {
      ...
      try {
        ...
        log.log(Level.WARNING,
                String.format(Constants.getString("Error.Start.Phase"), superLauncher.getPhase()),
                ex);
        if (superLauncher.checkIgnoreUpdateErrorSetting()) {
          superLauncher.reportError(String.format(Constants.getString("Error.Start.Phase"),
                                                  superLauncher.getPhase()),
                                    ex);
          System.exit(1);
        }
      }
  ...
  private void launchAppFromManifest(boolean showWhatsnew) throws Exception {
    ...
    runAndWait(() -> {
      try {
        if (showWhatsnew
            && superLauncher.getManifest().whatsNewPage != null) showWhatsNewDialog(superLauncher.getManifest().whatsNewPage);
  ...
  private void startApplication() throws Exception {
    if (app != null) {
      ...
      if (appparams == null) {
        final LauncherParams params = new LauncherParams(getParameters(),
                                                         superLauncher.getManifest());

        ParametersImpl.registerParameters(app, new ParametersImpl(params.getRaw()));
